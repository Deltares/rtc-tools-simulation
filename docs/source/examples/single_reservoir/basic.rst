...
In this example the `rtcDataConfig.xml` looks as follows.
...
.. literalinclude:: ../../../../examples/single_reservoir/input/rtcParameterConfig.xml
    :language: xml

The input file `timeseries_import.xml` is usually not created by the user,
but generated by a FEWS application.
Here is a snippet of the file used in this example.

.. literalinclude:: ../../../../examples/single_reservoir/input/timeseries_import.xml
    :language: xml
    :lines: 4-19

The line ``<timeSeries id="H_observed">`` in the `rtcParameterConfig.xml` points to the model variable is called ``H_observed``
and the lines ``<locationId>reservoir</locationId>`` and ``<parameterId>H.obs</parameterId>``
indicate that ``H_observed`` has locationId ``resrvoir`` and parameterId ``H.obs``.
...
The line ``<event date="2022-06-07" time="06:00:00" value="1598.32" flag="8"/>``
sets the value of ``H_observed`` at the initial time (2022-06-07, 06:00:00).
We only provide the reservoir elevation with an initial value in this example
